---
title: "Onyx 368 Shared Lab Inventory"
room: "Onyx 368"
building: "Onyx Hall"
lab_type: "Field Processing / Dirty Lab"
biosafety_level: "Non-molecular"
maintained_by: "IEE Shared Molecular Laboratory"
contact: "mcurrey@uoregon.edu"
last_updated: "2026-07-14"
description: >
  Inventory of equipment and benchtop station resources in Onyx 368,
  a shared field processing ("dirty") lab managed by the Institute of
  Ecology and Evolution (IEE) at the University of Oregon. This space
  is intended for sorting and initial processing of field-collected samples
  and supports limited molecular work. It is not a designated molecular lab —
  for BSL1 molecular work, see the 327 Pacific Hall inventory. This document
  tracks equipment locations and shared resources across workstations to
  support coordinated use of the space by IEE researchers.
---

## Big Equipment

| Equipment | Notes |
|-----------|-------|
| Drying Oven | |
| Freezer -20°C | Back wall |
| Centrifuge 5417C | Eppendorf |
| Fisherbrand Beadmill 24 | |
| Biological Safety Cabinet | |
| Molecular Fridge | Counter by shelves |
| Electronic Balances/Scales | x2, counter by shelves |
| Micromix Centrifuge | Station 4, front wall |
| Vortex Genie 2 | |
| Thermo Scientific Dionex ICS-2100 Ion Chromatography System | |

---

## Benchtop Stations

### Station 1

- Shared pipettes w/ Station 2
- Kits on top shelf
- Gloves shared w/ Station 2

### Station 2

- Shared pipettes w/ Station 1
- Kits on top shelf
- Gloves shared w/ Station 1
- **Note:** Currently being used for H. Dawson's soil pH processing
  - Falcon tubes

### Station 3

- Shared pipettes w/ Station 4
- Kits on top shelf

### Station 4

- Shared pipettes w/ Station 3
- Kits on top shelf
- Micromix Centrifuge
- Vortex Genie 2
- Electronic balances/scales (x2)

### Stations 5, 6, 7, 8

- Shared pipettes

---

## Back Counter

- 2 big beakers
- Centrifuge 5417C (Eppendorf)
- Fisherbrand Beadmill 24
- Tray
- Timer

## Counter by the Shelves

- Molecular fridge
- Electronic balances/scales (x2)

---

## Main Shelving Unit

| Shelf | Contents |
|-------|----------|
| Bottom shelf | Pipette tips (x2 boxes) |
| Next shelf up | Pipette tips |
| Next shelf up | Pipette tips, trays |

---

## Glass Cabinets

### Far Left Upper Cabinet

| Location | Contents |
|----------|----------|
| Top shelf | Small plastic containers of various sizes |
| Middle shelf | Petri dishes, plastic trays, aluminum weigh boats, plastic weigh boats, well plates |
| Lower | Containers of plastic tubes w/ metal beads, tubes, TCP tubes |

### Right Upper Cabinet

| Location | Contents |
|----------|----------|
| Top shelf | Misc equipment, Falcon tubes |
| Middle shelf | Misc equipment |
| Bottom shelf | Vacuum flask, misc centrifuge equipment, aspirating pipettes, spatulas, pipette pans |

---

## Bottom Cabinets

### Left Bottom Cabinet

- Glassware

### Cabinets by Sink

- Markers and post-its
- Totes
- Computer, keyboard + mouse
- Office supplies

### Right Bottom Shelf

- Misc. supplies
- Instruction manuals
- Tile plates
- Lamp protector

---

## Fume Hood

- Tube holders
- Drying rack
- Pipette holder
- Pipette